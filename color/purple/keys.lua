...
local function client_property(p)
	if client.focus then
		client.focus[p] = not client.focus[p]
	end
end
...
function hotkeys:init(args)
	...
	local apphelper = function(keys)
		...
		for name, sheet in pairs(keys) do
			if name == app then
				modtip:set_pack(client.focus.class, sheet.pack, sheet.style.column, sheet.style.geometry, function()
					modtip:remove_pack()
				end)
				modtip:show()
				return
	...
	local menu_keys_move = {
		...
		},
		{
			{ env.mod },
			"j",
			flex.menu.action.back,
			{ description = "Go back", group = "Navigation" },
		},
		{
			{ env.mod },
			"l",
			flex.menu.action.enter,
	...
	for i, k in ipairs(kk) do
		table.insert(keyseq[3][5][3], {
			...
			k,
			function()
				client_move_by_index(i, { client.focus })
			end,
			{},
		})
		table.insert(keyseq[3][6][3], {
			...
			k,
			function()
				client_toggle_by_index(i, { client.focus })
			end,
			{},
		})
		table.insert(keyseq[3][7][3], {
			...
			k,
			function()
				client_move_and_go_by_index(i, { client.focus })
			end,
			{},
		})
	end
	...
	table.insert(keyseq[3][5][3], {
		...
		line1_symb,
		nil,
		{ description = "Move client to tag on 1st line", group = grp, keyset = numkeys },
	})
	table.insert(keyseq[3][6][3], {
		{},
		line1_symb,
		nil,
		{ description = "Toggle client on tag on 1st line", group = grp, keyset = numkeys },
	})
	table.insert(keyseq[3][7][3], {
		{},
		line1_symb,
		nil,
		{ description = "Move client and show tag on 1st line", group = grp, keyset = numkeys },
	})
	table.insert(keyseq[3][5][3], {
		{},
		line2_symb,
		nil,
		{ description = "Move client to tag on 2nd line", group = grp, keyset = tagkeys },
	})
	table.insert(keyseq[3][6][3], {
		{},
		line2_symb,
		nil,
		{ description = "Toggle client on tag on 2nd line", group = grp, keyset = tagkeys },
	})
	table.insert(keyseq[3][7][3], {
		{},
		line2_symb,
		nil,
		{ description = "Move client and show tag on 2nd line", group = grp, keyset = tagkeys },
	})
	...
	for i, k in ipairs(kk) do
		table.insert(keyseq[3][10][3], {
			{},
			k,
			function()
				client_move_by_index(i, current_clients())
			end,
			{},
		})
		table.insert(keyseq[3][11][3], {
			{},
			k,
			function()
				client_toggle_by_index(i, current_clients())
			end,
			{},
		})
		table.insert(keyseq[3][12][3], {
			{},
			k,
			function()
				client_move_and_go_by_index(i, current_clients())
			end,
			{},
		})
	end
	...
	self.raw.root = {
		...
		},
		{
			...
			"c",
			function()
				flex.float.keychain:activate(keyseq, "User")
			end,
	...
	for i = 1, tcn do
		self.keys.root = awful.util.table.join(
			self.keys.root,
			tag_numkey(i, { env.mod }, function(_)
				tag_double_select(i, tcn)
			end),
	...
	for i, k in ipairs(tagkeys) do
		self.keys.root = awful.util.table.join(
			self.keys.root,
			awful.key({ env.mod }, k, function()
				tag_double_select(i + tcn, tcn)
			end),
			awful.key({ env.mod, "Control" }, k, function()
				tag_toogle_by_index(i + tcn)
			end)
		)
	end
